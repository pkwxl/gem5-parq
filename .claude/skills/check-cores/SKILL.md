...
退出码：`0` = 两条臂全空闲；`1` = 至少一条臂一个空闲核都没有；`2` = 部分空闲
（有可用核，但不是全部）。
...
- **忙是粘性的**（决策 0004 §3.1）：测到忙就假定它一直忙，直到下一次实测到空闲为止。
  想知道是否释放了，就再跑一次本 skill——不要靠推测「应该快跑完了」。
- **够用就开跑**：只要空闲核数量满足计划要求，就用那些空闲核，**默认不必顾虑对同机
  其他实验的影响**（除非计划或用户特别要求不打扰）。
- **默认允许跨臂借核**：优先用本臂自己的保留核；不够时可以借另一条臂的空闲核（串行臂
  只有 2 个核，很容易被占满），只借计划需要的数量。两条臂都不够才停下走 Checkpoint 2。
- 实际用的核号**及其 NUMA node** 必须写进 Checkpoint 1 和 spec 的结果章节。两处必记的
  已知扰动源：① 同机有别的实验在跑（绑核不隔离 LLC 和内存带宽）；② 跨臂借核会跨 NUMA
  node（串行臂 node 0、并行臂 node 1），与历史同臂数字**不是同一内存局部性条件**，比较
  时必须列为已知差异。
