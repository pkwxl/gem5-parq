...
- **够用就开跑**：只要本条臂的空闲核数量满足计划要求，就可以用那些空闲核，**默认不必
  顾虑对同机其他实验的影响**（除非计划或用户特别要求不打扰）。空闲核不足才停下走
  Checkpoint 2；**不跨臂借核**——两条臂在不同 NUMA node 上，换过去就不是同一个工作点了。
- 实际用的核号必须写进 Checkpoint 1 和 spec 的结果章节。同机有别的实验在跑时，一并记为
  本次数字的已知扰动源：绑核隔离了 CPU 占用，**没有**隔离 LLC 和内存带宽。
