...
def main() -> int:
    ...
    print(
        ...
        "空闲核够用就开跑，默认不必顾虑对同机其他实验的影响；本臂不够时默认可以跨臂借"
        "另一条臂的空闲核（只借计划需要的数量），两条臂都不够才停下走 Checkpoint 2。\n"
        "实际用的核号**及其 NUMA node** 必须写进 Checkpoint 1 和 spec，并记明两个已知扰动"
        "源：同机有别的实验在跑（绑核不隔离 LLC/内存带宽）、跨臂借核跨了 NUMA node"
        "（串行 node 0 / 并行 node 1，与历史同臂数字不是同一内存局部性条件）。\n"
