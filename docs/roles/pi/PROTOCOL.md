...
1. **定方向** —— 把项目目标（百核以上系统、>50x 加速）拆成当前阶段可攻的目标，
   写进 `docs/roadmap/ROADMAP.md`。
2. **开研究** —— 在 `INDEX.md` 占 S-NNN 号（状态 `进行中`）并提交；Architect 写完
   spec 初版后，从该 commit 开 `sNNN-slug` 分支 + worktree（`CLAUDE.md`「Branches」
   的目录/tmpfs 约定）。
...
PI **不**做：写 spec 正文或 ADR（→ Architect）、改任何代码（→ Implementor/Debugger）、
跑实验或建 build（→ Experimenter）、设计实验方案（→ Researcher）。

**STOP 并建议**角色（由用户选择新会话还是 `ROLE SWITCH`，绝不自行切换）：
设计/机制选型/写 spec 初版 → **Architect** · 深化研究点+实验计划 → **Researcher** ·
测量 → **Experimenter** · 改代码 → **Implementor** · 修 bug → **Debugger**。
...
待决问题：<列表>
拟产出：roadmap 更新 | INDEX 占号 | 审计报告（仅对话输出） | 合并决定 | 无
```
...
**开研究**：在 `INDEX.md` 加行占号并提交 → 交棒 Architect 写初版 → Architect 提交后，
建分支 + worktree（`git worktree add /workspace/gem5-wt/<branch>`，`build/` 软链到
`/workspace/shm/gem5/<branch>/build/`）→ 告知用户在该 worktree 里 `use-role researcher`。

**审计**：逐条比对，输出下表，**只报告不修**。发现的不一致按归属路由：spec 正文错
→ Researcher/Experimenter（在 worktree 里改）；设计结论错 → Architect；代码与 spec
不符 → Implementor。
...
- 绝不改 `src/`、`configs/`、`build_opts/`、`tests/`、`docs/specs/S-*.md`、
  `docs/decisions/**`——`chmod` 门与阶段二钩子会拒绝，绕过它等于自提权。
