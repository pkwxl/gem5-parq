# S-001: 设计背景与提案（parti-gem5 对照、开放问题、Throttle 死锁规避设计）

> **状态**：设计定稿。所有标记"待定"的问题（A-H）已在本文档内确认，已被
> [S-002](./S-002-v1-per-consumer-lock.md) 的 v1 实现落地。
> 属于 [parallel-EventQueue 项目](./INDEX.md) 的第 1 份 spec，对应原单体设计
> 文档（已拆分，原文件已删除）的 0-6 节，内容原样保留。
...
**上一篇**：（无——这是本项目最早的一份 spec）
**下一篇**：[S-002：v1 实现：per-consumer 锁](./S-002-v1-per-consumer-lock.md)
