...
> 这是一份**问题记录 + 实现/实测日志**，从设计前置材料一路记到实现落地——目的
> 是让后续新开的会话能够直接带着完整背景进入，不用重新梳理。0-6 节是设计讨论，
> 7 节起是逐会话的实现与实测记录。**阶段性结论见文末第 13 节**：方案成立、
> 内存安全、时序中性，且自旋屏障让并行 EventQueue 首次跑出对串行的真实加速。
...
1. **木桶效应（straggler）**：每个 quantum 末尾所有线程在全局屏障
   ...
   同一并行 repro 的 wall-clock 曾在 56-620s 间摆动（8.7 节的 7 连跑），
...
绑核是 9.3 出路 2 / 9.4 候选出路 3（自旋屏障替换 cv 屏障，
...
1. **内存安全**：SE 模式下每一个跨域竞态都被定位并修掉（Ruby functional 遍历
   加锁 9.7；futex UAF + 跨域 TLB flush + 跨域 deschedule 9.8），完整多线程负载
   ASan 干净跑到 tick 8.5e9（9.8）。FS 模式下 APIC 中断跨域唤醒墙（11.5，量子
...
屏障实测 ~25.6µs/quantum（9.2）。10 节的宿主线程绑核（PDES 调度纪律）解锁了
自旋屏障这个前置条件，12 节据此落地了**可选屏障模式**（cv/spin/hybrid），并
两侧实测：

- **每-quantum 屏障成本从 ~25.6µs 降到 ~3µs**——9.3 预测的整整一个数量级，在
...
这是**并行 EventQueue 路线第一次跑出对串行的真实、可复现加速**（此前 9.1 是
125x 减速）。纯 spin 是最大加速模式（绑核 + 无超线程 + 核数富余下自旋安全，
10.4），hybrid 是不均衡/被抢占场景的兜底。

**还没到头，但瓶颈已经换人。** 0.90x（SE）/1.41x（FS）说明屏障不再是主瓶颈后，
下一根杠杆是 9.3 出路 1/3——把跨域链路延迟建成更真实的值（抬高 Q 上限）、每域
放更多工作（摊薄固定同步成本）——而不是继续抠屏障或 7.3 那 ~15% 的单线程锁开销。
SE 模式的 OS/线程仿真天花板（9.9）仍是选 FS 做主战场的理由，这一点没变。
