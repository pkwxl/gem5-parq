...
> **当前基准数字（PI 更新，2026-07-22）——先读这一段，再读下面的历史。**
> 下面"现状（截至 S-009）"那一大段是逐次会话追加形成的历史记录，其中反复
> 出现的 **0.91x（S-009 §27）已作废**：S-013 §9 事后查明它测自
> `MAX_TICKS=2e8` 的窗口，只跑了 74588 条指令、大概率还在客户机线程启动
> 阶段，不构成对当前工作点的有效测量。
>
> 以 **S-017** 为准——首个真实小时尺度、均衡检查点、tracing 关闭的干净三臂
> 数字：**overhead ratio ≈ 1.002 · real speedup ≈ 0.459x · internal speedup
> ≈ 0.460x**，即并行臂比 baseline **慢 2.18 倍**。S-017 §6 已把这 915.67s
> 的差距定量归因完毕：跨域屏障到达时间差总和 1097.34s（差距的 1.20 倍），
> 其中 ~73% 是 4.024 亿次跨域同步每次 1–4 微秒的固定税，~16% 是 847 次
> 长尾停顿；锁竞争 <0.15%（S-012 §19），不是瓶颈。
>
> 当前优先级与阶段目标见 [../roadmap/ROADMAP.md](../roadmap/ROADMAP.md)。
...
| S-012 | [S-012-eventq-critical-path-instrumentation-design.md](./S-012-eventq-critical-path-instrumentation-design.md) | **用户确认按 5 步实现；Step 1-5 全部完成；§19 有界重跑显示结论随窗口长度漂移，出路 3 仍缺收敛基线**（专属分支
`s012-eventq-critical-path-instrumentation-design`）；§15 长窗口段错误已修复+验证（§16）：真正根因不是 §15 猜的 map 并发（加锁后同样复现，已用 gdb 证伪），是 `critPathFlush()` 的 `std::atexit` 注册和主线程自己 `critPathBuffer` 的 `thread_local` 析构之间的 LIFO 时序竞争（确定性 UAF）+ 子线程在正常退出路径上从未被 join（`terminateEventQueueThreads()` 以前只接在 `fork()` 上）；改成在 `m5/simulate.py` 用 `atexit.register()` 显式调用，绕开了 C++ 层的时序赌局；长窗口重跑不再崩溃、8 份 CSV 全部生成，正确性回归（无 critpath tracing）逐字节相同，TSan 干净但顺带曝光 3 条跟这次改动无关的 `Consumer::commitTick` 跨域调度既有竞态（未深入，候选新 S-NNN）；**Step 5（§17）用 S-013 §11 已经在均衡检查点上跑出的真实长窗口（`MAX_TICKS=2e9`，300300 quanta）数据正式收编为关键路径分析交付物**：均衡检查点上的正确性回归+TSan 回归均干净（TSan 3 条警告和 §16 那次同一组签名，跨检查点复现，确认是既有缝隙不是新问题，也顺带补上了 §14.3 记录的"Step 4 四个文件需要专门 TSan"待办）；三个未知数正式作答——未知数 1 是域 1/3/4 三个核私有域共同分担（21-31% 各），不是单域主导；未知数 2 **修正了 §14.6 的结论**——`CacheLock` 在域 1/4 占 ~11% hostSeconds，不再是"几乎可忽略"，锁竞争是真实但次要的成分；未知数 3 域 1/3/4 最后到达时 `eventCount` 是域 0/2 的 27-28 倍，不均衡确认存在；**唯一未完成项**：Step 5 计划要求的真实长窗口下 2-3 次重复跑评估直方图稳定性没有做（只有短窗口的重复跑，S-013 §7，代表性存疑）；**§18（同均衡检查点，无 `MAX_TICKS`，70 分钟 `timeout` 安全网）**：拿到 1039 个周期的 wall-clock 吞吐数据（稳态 ~7.4×10⁸ tick/s，长跑无系统性下降，但 17 个周期出现 5-121 秒的未解释长尾停顿，占真实时间 12.4%），**没拿到**关键路径 CSV——新发现的操作性缺口：`timeout` 默认发 `SIGTERM`，Python 对此无默认处理器，直接终止不经过 `atexit`，而 `critPathFlush()`（§16 修法）正是靠 `atexit` 挂的，所以 CSV 没落盘（`stats.txt` 周期性 dump 是同步写入、不受影响）；跟真正的 hour-scale 直方图稳定性验证仍是两回事，需要改用有界 `MAX_TICKS` 走干净退出路径才能补上；**§19（有界 `MAX_TICKS=1.34e12` 重跑，用户明确要求"保存~124GB数据到 `/workspace/shm/gem5/...`"）**：CSV 拿到了（8×~14GB），但 `critpath_aggregate.py` v1/v2 在 32 亿行规模下不可用（会把 groups 撑到几百 GB），改用 awk/paste/sort 管道重新实现；201,201,201 个 quantum 的真实结果**再次方向性反转**——最后到达者份额在 8 个域间摊平（6.9%-16.6%），§17.3"域 1/3/4 主导、域 2 偏低"不成立；锁等待占比从 §17.3 的 ~11% 掉到 <0.15%；两个反转共同指向"关键路径画像可能随窗口长度系统性漂移，2e9-tick 窗口本身可能都不够长"这个更基础的怀疑；**意外定位到 §18 长尾停顿的根因**：`critPathBuffer`（`thread_local std::vector`）未设 `EVENTQ_CRITPATH_RESERVE`，扩容拷贝耗时随记录数越过 2 的幂次同步翻倍，8 个已知停顿周期里 8 个吻合这个模型（294、778-780 仍未解释）——是插桩自己的开销，不是主机噪声或并行 EventQueue 机制本身的性质 | 出路 3（每域塞更多工作）立项前缺一份关键路径数据——把这个缺口拆成三个可测的未知数：哪个域最后到达 quantum 屏障（谁在关键路径上）、每个线程墙钟时间构成（处理事件 vs 等屏障 vs 阻塞在跨域锁上）、各域每 quantum 工作量是否不均衡；设计了两类打点（`Barrier::wait()` 到达/离开时刻 + `UncontendedMutex` 慢路径等锁时长，四个已知跨域锁 `layerLock`/`pioLock`/`cacheLock`/`m_wakeup_mutex` 打标签）加一个可选第三类（每域每 quantum 事件数，用来区分"真的多干活"和"卡在锁上")；关联键初稿误用了 `simQuantumStart`（它是运行期常量——S-009 §22 论证其跨线程读安全的根据恰恰是"不变"，按它分组会把整个运行归成一组），评审后修正为各域自己队列在打点处的本地 `curTick()`（同一 quantum 边界各域相同、纯线程本地读，§3.4）；域号通过线程入口的 `thread_local` 变量落地（`simulate.cc` 的 `thread_main`），不做每次打点查表；关闭时对已验证的快路径行为无改动（`UncontendedMutex` 多一次可预测分支），打开时是独立诊断跑、不产出可比性能数字；**Step 3（§13）第一次打开插桩跑真实窗口**：域 1（核 0 私有 L1+L2）是压倒性最后到达者（50.4%），推翻了"共享 L3/目录域大概率更重"的原始直觉；"为什么是域 1"留白，由 **S-013** 接手 |
| S-013 | [S-013-balanced-four-core-workload-checkpoint.md](./S-013-balanced-four-core-workload-checkpoint.md) | **部分正面结果——新检查点已建+已验证；§7 短窗口测得的"域 3 一家独大"被 §11 的真实长窗口数据推翻其量级（§9 早已指出 §7 的窗口大概率还在客户机启动阶段）：真实尺度下不均衡分散在域 1/3/4 三个域（各 21-31%），不再是单域压倒性主导，但域 2 异常偏低（3.9%）是新的、未解释的不对称；§10 记录了长窗口校验一度被 S-012 自己插桩基础设施的一个新段错误挡住，§11 是该段错误（S-012 §16）修复后重新聚合数据的结果；锁等待数据（S-012 Step 4）显示域 1/4 的 `CacheLock` 等待占 `hostSeconds` ~11%，可能是不均衡的部分成因，未深入；最有价值的下一步是解释域 2 为何持续偏低** | S-012 §13.5 留白的"为什么域 1 是关键路径"：读 `threads.cpp` 源码定位到一个真实根因——基准的数组初始化+结果校验是全串行的（40 万元素操作），全部落在主线程所在的核上，且从无显式线程绑核；写了 `threads_balanced.cpp`（三阶段全并行+每线程显式绑 vCPU，`chunk_size`/`num_values` 参数不变以控制变量），原生沙箱内验证正确性；新增 `x86_fs_classic_save_ckpt_balanced.py`，跑通新检查点（~5.7 小时，同 S-006 §11.1 量级）；用 S-012 §13 同一套关键路径协议验证——**不均衡以几乎同样的幅度持续存在，只是从域 1 换成了域 3**（时间序列显示是逐渐"滚雪球"变大，不是恒定），推翻了"平摊三阶段负载就能均衡"的假设；重跑同一检查点做确定性检验，域 3 两次都赢（51.2%/52.6%），**排除了纯随机/混沌解释，指向某种未定位的结构性原因**；验证过程中还发现并妥善处理了一个待决问题——工作树里有一份未提交、形状与 S-012 Step 4（锁打标签）一致的改动，用路径级 `git stash` 隔离出干净的 Step-3 build 做对照，验证完后已还原 WIP；**§9 追问"窗口是不是太短"发现 `MAX_TICKS=2e8` 只够跑 74588 条指令、大概率还在客户机线程启动阶段**，拉长窗口（去掉 tick 上限）复现了一个和本文主题无关但严重得多的问题，见 **S-014** |
...
| S-017 | [S-017-balanced-bounded-three-arm-hostseconds.md](./S-017-balanced-bounded-three-arm-hostseconds.md) | **已完成，已合并进 `main`（`--no-ff`，`4d5889c474`；分支 `s017-balanced-bounded-three-arm-hostseconds` 与其 worktree 均保留未删除，`/workspace/shm/gem5/s017-.../` 下的 ~125GB critpath 原始数据也保留在 tmpfs 上、重启即失）**（三臂在均衡检查点、`MAX_TICKS=1.34e12`、tracing 关闭的条件下全部干净跑完；纯文档投研，无代码改动） | 承接 S-012 §19.4（锁等待占比在真实小时尺度下 <0.15%，可忽略）拿到了 CLAUDE.md"三臂对照"方法论本身在均衡检查点、真实小时尺度上的首个干净数字：**overhead ratio ≈ 1.002**（串行模式下跨域锁开销几乎可忽略，与 §19.4 一致）；**real speedup ≈ 0.459×**、**internal speedup ≈ 0.460×**——parallel 臂反而比 baseline/current-serial 慢约 2.18×，与项目"真实提速"的目标相反，且比 S-009 §27 短窗口测得的 0.91× 慢得多；三臂 `simInsts` 在终点（tick 6,643,194,840,323）逐位相同，无崩溃/断言/段错误，670/670 周期性 dump 全部落地，均在 3 小时 `timeout` 安全网内（实际各不到 25 分钟）自然完成，未触发安全网。`hostSeconds` 需要按 670 个周期求和而非取最后一次 dump（`simSeconds` 每次 dump 都固定在单周期的 0.002s，证实该字段每周期重置，不是累积值）。**§6 同会话追加**：current-parallel 单独开 `EVENTQ_CRITPATH_TRACE=1` 重跑一次（数据落 `/workspace/shm/gem5`，378GB tmpfs，8 份 CSV 共约 118GB），沿用 S-012 §19.2 验证过的按行位对齐技巧（8 个域 barrier 行数逐位相同，均为 402,402,402，与 S-012 同配置下的行数吻合）算出 `spread_ns`（跨域到达时间差）分布：**总和 1097.34s，是 915.67s（parallel−baseline）差距的 1.20 倍**——屏障忙等本身就足以解释、甚至超出这次测到的减速，不需要另外假设"锁"或其他机制。其中约 73%（~900s）来自几乎每次跨域同步（4.024 亿次里 98.5%）都要付的 1-4 微秒级"固定税"，约 16%（~200s）来自极罕见但巨大的长尾停顿（847 次，最大单次 10.16 秒）——与 S-012 §18 那批"未解释长尾停顿"是同一类现象的第二次独立观测，仍未查明根因 |
| S-018 | [S-018-o3-cpu-restore-balanced-checkpoint.md](./S-018-o3-cpu-restore-balanced-checkpoint.md) | **已结题**（PI 决定，2026-07-22：O3 从 Atomic 检查点 restore 的路径已跑通，标题任务完成；`m_buf_msgs` 分歧的根因已定位、修复移交 **S-019**；「O3 能带来多少跨域并发收益」是一个新问题，另行立项，不留在本号下）（专属分支 `s018-o3-cpu-restore-balanced-checkpoint`，已合并进 `main`（`--no-ff`）作为里程碑，分支保留）：构建干净、O3 从 Atomic 检查点 restore 成功不崩溃；短窗冒烟测试的 serial/parallel `stats.txt` diff 出真实分歧——若干条 `MessageBuffer::m_buf_msgs`（时间加权平均统计）只在 parallel 臂炸到 1e12 量级；对照组（未改动的 TimingSimpleCPU 脚本，同一极短窗口）diff 为空、逐字节相同，排除"短窗口本来就容易出这类假象"；隔离测试（`max_outstanding_requests` 改回默认 16）分歧依然复现，排除是这次调参引入的。**§6.2：根因已定位**——迄今观测到的全部 6 个假值都精确等于 `k * 2^64 / 10000001`（`k ∈ {1,2,3,5}`），分母完全正确，因此**推翻**了此前"分母取错域的 `curTick()`"的假说；溢出在分子：`AvgStor::set()`（`src/base/stats/storage.hh`）执行 `total += current * (curTick() - last)`，`Tick` 是 `uint64_t` 而 `last` 由最后碰过该 buffer 的那个域盖戳，在松弛跨域计时下落后的域更新共享 `MessageBuffer` 时 `curTick() < last`，相减下溢成 ~2^64，再乘以当时的 buffer 占用数（即 k）。属 O3 触发而非 O3 引入——是 S-009~S-016 同一族的跨域读，位于共享的上游 stats 代码。**尚未修复**：改 `AvgStor` / 改 `MessageBuffer` 调用点 / 给跨域共享统计一个域一致的时钟源，三者选哪条会影响此前每个 S-NNN 的统计数字，按约定留作 checkpoint，建议另开 S-019。（另：本次清理还原了被前序会话误改的共享 Timing 脚本 `x86_fs_mesi3_parallel_eventq.py`——日志前缀被错标成 `-o3`，且 `HOST_PIN_CPUS.replace("..", ",")` 会把 `100..107` 静默解析成 `[100, 107]` 两个核而非八个） | 用户指出 `TimingSimpleCPU`（当前所有 ROI 跑法用的 CPU 模型）逐条阻塞取内存、基本不产生同核并发在途请求，体现不出多 `EventQueue` 拆分想暴露的跨域并发收益；计划把 ROI 的 CPU 模型换成 `CPUTypes.O3`（乱序、多条在途 load/store），复用 S-013 现成的均衡检查点（`x86-threads-balanced3-roi-classic`）而不重新起跑几小时的新检查点——gem5 检查点只存架构态，和计时模型无关，`x86_fs_mesi3_parallel_eventq.py` 现在的 `ATOMIC` 启动/`TIMING` restore 分离本来就是这个套路，换成 `O3` restore 是同一模式的延伸；顺带审计了限制同核在途请求数的两个参数——`RubySequencer.max_outstanding_requests`（默认 16，本 fork 从未覆盖过）比 O3 默认的 `LQEntries`/`SQEntries`（32/32）更紧，是比 O3 自己的队列更早卡住并发度的那道闸，计划把它提到 32 跟 LSQ 对齐；`RubyController.number_of_TBEs`（默认 256）/`buffer_size`（默认无限）都够宽裕，不是瓶颈 |

| S-019 | [S-019-avgstor-crossdomain-tick-underflow.md](./S-019-avgstor-crossdomain-tick-underflow.md) | **进行中**（PI 于 2026-07-22 占号；Architect 初版 + [决策 0002](../decisions/0002-avgstor-backward-tick-clamp.md) 已落 `main`（`1faa10932c`）；专属分支 **`s019-avgstor`** 与 worktree `/workspace/gem5-wt/s019-avgstor` 已创建（2026-07-22 按[决策 0005](../decisions/0005-short-branch-names.md) 由 `s019-avgstor-crossdomain-tick-underflow` 改名，worktree 与 tmpfs build 一并搬迁，5.7G 构建产物保留未删），build 软链到 `/workspace/shm/gem5/s019-avgstor/build`；分支上已有三次角色会话的实质产出——Researcher 的机制分析 + 预注册实验计划（`dfc71d2abf`）、Experimenter 记录 E0 **无法执行**（`c81087973f`：`role-gate.py:356` 对 experimenter 拒绝 `scons`，零结果数据）、Implementor 按 ADR 0002 实现的 `AvgStor` clamp + 插桩（`7b01be5f0a`，**从未编译过**）。**PI 2026-07-22 复核后决定暂不合并**：未编译的共享 stats 改动不进研究主干，且分支另有三个越界提交（`.qwen/**`、`.gitignore`，不在任何 worktree 角色写权内）不予追认。顺序改为——先开一次 Architect 会话在 `main` 上裁定两个协议缺口（① experimenter 能否跑 `scons`，或把构建改派 Implementor 并修订 spec §9.1 的强制次序；② 分支如何从 `main` 取更新：`git merge` 是 PI 专属而 PI 只在主树工作，目前仅 `rebase` 未被 hook 拦，属机制漏网），再在 worktree 里 rebase 到新 `main` 继续 E0）：承接 S-018 §6.2 已完成的根因定位——`AvgStor::set()`（`src/base/stats/storage.hh`）执行 `total += current * (curTick() - last)`，`Tick` 是 `uint64_t`，而 `last` 由最后碰过该统计对象的那个域盖戳；在本 fork 松弛的跨域计时下，落后的域更新共享 `MessageBuffer` 时会出现 `curTick() < last`，相减下溢成 ~2^64，再乘以当时的 buffer 占用数 k，产出精确等于 `k * 2^64 / 10000001` 的假值（已观测 6 个，`k ∈ {1,2,3,5}`）。**影响面比 O3 大得多**：这是共享的上游 stats 代码，并行臂的任何时间加权统计（`AvgStor` 系）都因此不可信，会污染出路 3 的全部测量——这是它排在出路 3 之前的唯一理由。三条候选修法（改 `AvgStor` / 改 `MessageBuffer` 调用点 / 给跨域共享统计一个域一致的时钟源）选哪条会影响此前每个 S-NNN 的统计数字，属设计决策，需 Architect 先出 ADR | 承接 S-018 §6.2；目标是让并行臂的时间加权统计可信，本身不产出加速比数字 |

**S-019 更新（PI，2026-07-23）**：分支上 §16-21 把 Ruby 网络侧钳位（`MessageBuffer`/`Throttle`/`NetworkInterface`）也做完并验证触发（A0 §21.4 观测到 15 条假值行，`k` 范围 1-24，比 S-018 观测到的更广），但 §21-22 的 A3/A4（O3 长窗口校准）撞上一个**独立于 AvgStor** 的确定性崩溃——已分离为 **[S-022](./S-022-o3cpu-scheduletickevent-crossdomain-crash.md)**（`O3CPU::scheduleTickEvent` 缺跨域 grid-anchored snap）。S-019 的 A3/A4 在 S-022 修复前保持阻塞；V2/V3 检测器本身的功能验证（§19-21）不受影响，已完成。

| S-020 | [S-020-numa-node-offset-serial-arm.md](./S-020-numa-node-offset-serial-arm.md) | **进行中**（PI 于 2026-07-22 占号；spec 正文尚未写，Architect 初版已落 `main`（`83ad4f693e`）；专属分支 **`s020-numa`**（按决策 0005 的新命名）与 worktree `/workspace/gem5-wt/s020-numa` 已于 2026-07-22 建立，build 软链到 `/workspace/shm/gem5/s020-numa/build`；下一步是 Researcher 深化 R1 并写实验计划）；优先级 **P2**——不产出加速比数字，是给 [决策 0004](../decisions/0004-reserved-cores-single-source-and-idle-check.md) §3.1「跨臂借核默认放行」做量化背书的方法学校准。**升级触发器**：一旦某次实测出现无法用其他因素解释的 node 0 / node 1 差异（ADR 0004 §6 的重审条件），本号立刻升 P1，因为届时所有用过跨臂借核的历史数字都要重新审视 | 承接决策 0004 §3.1：保留核跨 NUMA node（`SERIAL_ARM_CPUS` 在 node 0、`PARALLEL_ARM_CPUS` 在 node 1），协议已默认允许串行臂借用并行臂的空闲核，但**借核带来的 `hostSeconds` 偏移方向与幅度从未测量**——协议只要求「记录 NUMA node」，那让差异可见，不等于知道它有多大。目标是把这个偏移量测出来。**范围限制（占号时即已确认，不要事后重新发现）**：本号只能测**串行臂**的 node 敏感度——并行臂要 8 个核，而 node 0 的保留核只有 2 个，把并行臂搬到 node 0 在保留核范围内不可行，借非隔离核引入的噪声大于待测量本身。四个研究点（留给 Researcher 深化；**R4 是 Architect 初版补入、PI 2026-07-22 接受的范围扩展**，纯文档审计不占机时）：R1 偏移的量级/方向/重复离散度；R2 偏移是否随窗口长度或工作负载漂移（本项目已被「结论随窗口长度漂移」坑过三次：S-012 §19、S-013 §9、S-014）；R3 偏移对**比值**（Real speedup）的影响是否小于对绝对值的影响——若成立，跨臂借核的实际风险比 0004 §3.1 记的小；R4 **历史 pin 核清点**——逐个 spec 核对实际用过的核号与其 NUMA node，标出哪些结论是跨 node 比较出来的（写初版时已核出一例：S-017 §2.2 的三臂实际**全在 node 1**，而 S-008/S-009 的串行臂在 node 0；决策 0004 §3.1 已据此订正，并说明 S-017 内部比值不受影响、风险在跨 spec 比较）。**口径注意**：这不是加速比实验，`CLAUDE.md` 三臂制在此**不适用**，headline 是 node1/node0 的 `hostSeconds` 比值及其重复性。**已知排期约束**：R1 必须用 node 0 的串行保留核，这是唯一**不能**靠跨臂借核绕开的实验，占号时 `54-55` 正被另一次三臂测量占满 |

| S-021 | S-021-balanced-checkpoint-topology-ladder.md（**待 Architect 写初版**） | **进行中**（PI 于 2026-07-23 占号；spec 正文尚未写，分支/worktree 待 Architect 初版落盘后再建）；优先级 **P0**，排在 S-022 之前——用户直接指示：与其继续在当前 8-eventq（4 核私有 L1/L2 + 共享 LLC/目录）拓扑上调试（S-019 §19-23 反复暴露"太复杂，一个 bug 挡着另一个 bug"），先建一套可控复杂度的 balanced checkpoint 测试阶梯。**范围（用户已定，Architect 写初版时据此展开）**：第一步简化到 `1 核 + 1 L3` 的 2-eventq 拓扑（最小可用配置，先打通全流程）；第二步逐步扩展到 `2/4/8/16 核 + 1 共享 L3` 的 N+1-eventq；第三步再过渡到 `2/4/8/16 核 + 4 L3/MEM/...` 的 N+4-eventq（当前项目实际使用的拓扑形状）。每一级台阶都需要对应核数下"负载均衡"的检查点（S-013/S-017 已建的 4 核均衡检查点是这条阶梯里的一个既有参考点，不是终点）。**不产出加速比数字**，是方法学/调试基础设施投入——为 S-022 及后续所有跨域调度类 bug 的复现/调试提供比当前 8-eventq FS 拓扑更小的复现面 | 承接 S-019 §19-23 反复暴露的"当前 8-eventq 拓扑调试太复杂"这一操作性痛点；目标是让后续调试/测量可以先在最小复杂度台阶上复现，逐级验证再放大，而不是每次都要在最复杂的拓扑上定位问题 |

| S-022 | S-022-o3cpu-scheduletickevent-crossdomain-crash.md（**待 Architect 写初版**） | **进行中**（PI 于 2026-07-23 占号；spec 正文尚未写，分支/worktree 待 Architect 初版落盘后再建）；优先级 **P1**，按用户明确指示排在 S-021 之后（"在解决这个问题之前，我们应该构建一个更完善的测试方法"）。**来源**：[S-019](./S-019-avgstor-crossdomain-tick-underflow.md) §22-23（Experimenter/Researcher，2026-07-23）——S-019 的 A3/A4 校准跑在 O3 CPU + parallel + `x86-threads-balanced3-roi-classic` checkpoint 组合下，无论 `MAX_TICKS` 多短、无论二进制是否已含 AvgStor 钳位修法，都在同一个 tick（`5303863239922`）确定性崩溃于 `eventq.hh:784 assert(when >= getCurTick())`。**根因（S-019 §23 已定位，中高把握，未修复）**：`src/cpu/o3/cpu.hh:130-137` 的 `O3CPU::scheduleTickEvent()` 直接 `schedule(tickEvent, clockEdge(delay))`，没有像 `src/cpu/simple/timing.cc:219-245`（`TimingSimpleCPU::activateContext`，S-006 §11.5 已修）那样做 `inParallelMode && curEventQueue() != eventQueue()` 判断+grid-anchored snap；`clockEdge()` 用调用线程（跨域中断投递路径）的 `curTick()` 算，目标队列却是 O3CPU 自己的域，域 0 一旦领先超过算出的延迟即触发 assert。**不是** S-009/S-014/S-015/S-016 那条 `PacketQueue`/`BaseXBar` 链路的遗留（那条链路当前源码确认已全部修复，S-019 §23.1 逐一核对过）——是同一 bug 家族（"为单 EventQueue 世界写的代码，不知道自己可能被跨域调用"）在 `O3CPU` 自己的 `activateContext` 路径上一个从未被审计到的新点。**阻塞**：S-019 的 A3/A4（O3 校准/长窗口验证）在本号修复前保持阻塞。设计可直接复用 S-006 §11.5 / S-009 §19.2 已验证的 grid-anchored snap 模式，落点是目前这条 bug 家族里最小的一次改动 | 承接 S-019 §22-23；目标是让 O3 CPU + parallel EventQueue 组合不再确定性崩溃，解除 S-019 A3/A4 的阻塞；本身不产出加速比数字 |

## 如何新增一份 spec

1. 取下一个未使用的编号（当前最大是 `S-022`，下一份是 `S-023`），**不要**
   回填或重排已有编号，哪怕新主题在逻辑上是某个旧编号的延续。
   **并行开发下先在 `main` 上占号**：编号是全局分配的，两个并行调查不能
   同时抢同一个 `S-NNN`。开分支前，先在 `main` 上给下面的表格加一行
   （状态填 `进行中`，附上分支名 `sNNN-<word>`），提交——这一步在主干上把
   编号原子地占住，也让在途的调查可见。然后再
   `git worktree add /workspace/gem5-wt/sNNN-<word> -b sNNN-<word> main`。
   **分支命名见 [../decisions/0005-short-branch-names.md](../decisions/0005-short-branch-names.md)**：
   `sNNN-<word>`——一个小写单词、总长 ≤16 字符（`s019-avgstor`、`s020-numa`），
   **不再照抄 spec 文件名的 slug**；spec 文件名保持长描述性 slug，两者用**编号**连接。
