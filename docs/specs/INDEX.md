...
| S-016 | [S-016-unbounded-serial-run-hang.md](./S-016-unbounded-serial-run-hang.md) | **进行中——未定位根因，用户选择先搁置** | 检查另一会话跑的 S-014/S-015 无上限 serial/spin A/B 时意外发现串行臂挂起（`utime` 冻结在 ~37-40 CPU 秒、`wchan=futex_wait_queue`、`stats.txt` 空）；四次独立复现排除了 TSan、wrapper 脚本、`MAX_TICKS` 有无、S-009 至 S-015 整条线的跨域锁改动（`layerLock`/`pioLock`/`cacheLock`/`pqLock`）、检查点内容（换了 S-013 的 balanced 检查点一样挂）——只剩下一个不变量：冻结点永远是 `interrupts.cc:530` 那行只触发一次的广播 IPI `hack_once`；追踪 `intRequestPort.sendMessage()`→`PacketQueue::schedSendTiming()` 这条路径没找到重入锁问题（同一 tick 的多目标广播不会重复进 `EventQueue::schedule`），但顺着 `requestInterrupt()→wakeup()→activateContext()` 追下去，正是 S-006 §11.2-11.5 已经修过一次 bug 的同一条调用链（S-006 的修法只覆盖并行模式）；**更重要的意外发现**：回读 S-006 §11.3/S-007 §12.6/S-008 §16 才发现本项目从未真正确认过这份检查点的无上限串行跑能跑完——S-006/S-007 两次都留了"仍在 ROI 中/TBD"，S-008 转述的"跑完了"（`simInsts=3278585986`，~4.68 小时）本身就标注为宿主重启后无法核实的二手数据，S-009/S-014/S-015 往后所有"历史串行参考"都只是 `MAX_TICKS=2e9` 定窗的落点，不是真正跑到底；今天这四次挂起完全可能不是新引入的回归，只是第一次有人去查这类跑的进程级 CPU 活动而不是放着不管；最佳未确认假设——某处 `UncontendedMutex`（stock gem5，2020 年 Google 版权头，早于本 fork）自重入死锁；沙箱 `ptrace_scope` 挡住 gdb/strace，无法拿 backtrace 收尾；两个 bisection worktree（`bisect-a3e325a-serial-hang`、`bisect-3a03068-pre-s009-serial-hang`）留存未清理 | 见上，本行即摘要 |
