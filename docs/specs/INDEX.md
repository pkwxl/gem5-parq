...
| S-018 | [S-018-o3-cpu-restore-balanced-checkpoint.md](./S-018-o3-cpu-restore-balanced-checkpoint.md) | **进行中**（专属分支 `s018-o3-cpu-restore-balanced-checkpoint`；构建干净、O3 从 Atomic 检查点 restore 成功不崩溃，但短窗冒烟测试的 serial/parallel `stats.txt` diff 出真实分歧——两条 `MessageBuffer::m_buf_msgs`（时间加权平均统计）在 parallel 臂炸到 ~1.8e12/3.7e12；对照组（未改动的 TimingSimpleCPU 脚本，同一极短窗口）diff 为空、逐字节相同，排除"短窗口本来就容易出这类假象"；隔离测试（`max_outstanding_requests` 改回默认 16）分歧依然复现，排除是这次调参引入的——是 O3 本身（或 O3 触发了共享代码里既有的跨域漏洞），且两次跑的假值大小不同、涉及的 buffer 集合也不完全一样（三条 vs 两条），不是固定 sentinel；根因未追，见 §6-7） | 用户指出 `TimingSimpleCPU`（当前所有 ROI 跑法用的 CPU 模型）逐条阻塞取内存、基本不产生同核并发在途请求，体现不出多 `EventQueue` 拆分想暴露的跨域并发收益；计划把 ROI 的 CPU 模型换成 `CPUTypes.O3`（乱序、多条在途 load/store），复用 S-013 现成的均衡检查点（`x86-threads-balanced3-roi-classic`）而不重新起跑几小时的新检查点——gem5 检查点只存架构态，和计时模型无关，`x86_fs_mesi3_parallel_eventq.py` 现在的 `ATOMIC` 启动/`TIMING` restore 分离本来就是这个套路，换成 `O3` restore 是同一模式的延伸；顺带审计了限制同核在途请求数的两个参数——`RubySequencer.max_outstanding_requests`（默认 16，本 fork 从未覆盖过）比 O3 默认的 `LQEntries`/`SQEntries`（32/32）更紧，是比 O3 自己的队列更早卡住并发度的那道闸，计划把它提到 32 跟 LSQ 对齐；`RubyController.number_of_TBEs`（默认 256）/`buffer_size`（默认无限）都够宽裕，不是瓶颈 |
