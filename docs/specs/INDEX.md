...
**现状（截至 S-008）**：S-007 的"里程碑"是**并行屏障模式之间**的加速
...
项目目标的真实加速，出路 1 和出路 3（每域塞更多工作）大概率要一起做。下一
步杠杆是 S-009——把 Q 上限从 classic iobus 边（300）抬到 Ruby 合法值
（~6660）——投影结果需要在那之后重新实测验证。仍然悬而未决、不阻塞当前
结论的项：TLB 加锁后的热路径开销未测量、精确模式下一个恒定的一 ruby 周期
偏差未查明根因（S-006 §9.6 附近提到）。FS 完整 ROI 串行参考的 `simTicks`
曾经跑完过，但原始 `stats.txt` 已随一次宿主机重启从 `/tmp` 丢失，只剩交接
文档里未核实的转述数字（S-008 §16）——如果需要拿这几个数字做定量结论，
应重新跑一次。
