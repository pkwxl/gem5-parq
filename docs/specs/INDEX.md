...
| S-017 | [S-017-balanced-bounded-three-arm-hostseconds.md](./S-017-balanced-bounded-three-arm-hostseconds.md) | **进行中**（占号，专属分支 `s017-balanced-bounded-three-arm-hostseconds`；本次提交只有实验计划，尚未实际跑） | 承接 S-012 §19.4（锁等待占比在真实小时尺度下 <0.15%，可忽略）——既然锁不是瓶颈，该去拿 CLAUDE.md"三臂对照"方法论本身的真实数字了：均衡检查点（`x86-threads-balanced3-roi-classic`）、`MAX_TICKS=1,340,000,000,000`（沿用 S-012 §19.1 定的规模），**不开** `EVENTQ_CRITPATH_TRACE`（S-012 §19.4 已经指出插桩本身的 `hostSeconds` 不能拿来比较吞吐），baseline/current-serial/current-parallel 三臂各跑一次拿 `hostSeconds`，算出 overhead ratio / real speedup / internal speedup 三个数。核分配用 `100-111`（当前隔离核里 `54-55`/`92-99` 被另一个进行中的无上限三臂跑占着，`100-111` 是唯一还空着的隔离子集）：parallel 8 域占 `100-107`（沿用 S-012 §19.1 原样的分配），baseline 占 `108`，current-serial 占 `109`，`110-111` 留空。用户要求这次只起草 spec 并提交，实际发起进程放到下一个会话 |
